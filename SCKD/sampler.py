...
class data_sampler(object):
    ...
    def load_data(self, file):
        ...
        for sample in samples:
            ...
            self.id2sent[len(self.id2sent)] = tokenized_sample['ids']
...
class llm_data_sampler(object):
    ...
    def load_data(self, file):
        ...
        for _, sample in enumerate(samples):
            ...
            att_mask_0 = self.tokenizer(new_text,
                                    padding='max_length',
                                    truncation=True,
                                    max_length=self.config.max_length)['attention_mask']
            
            inputs = self.tokenizer(new_text + f' {self.id2rel[sample[0] - 1]}',
                                    padding='max_length',
                                    truncation=True,
                                    max_length=self.config.max_length)
            
            tokenized_sample['att_mask_0'] = att_mask_0
