...
class Bert_Encoder(base_model):
    ...
    def forward_mixup(self, inputs):
        ...
        if self.pattern == 'standard':
            ...
            tokens_output = self.encoder(inputs['ids'], attention_mask=inputs['mask'])[0]
    ...
    def forward(self, inputs):
        ...
        if self.pattern == 'standard':
            ...
            #  the representation of[CLS] mark.
            # output = self.encoder(inputs)[1]
            output = self.encoder(inputs['ids'], attention_mask=inputs['mask'])[0] # (b, max_length, h)
        else:
            e11 = []
            e21 = []
            # for each sample in the batch, acquire the positions of its [E11] and [E21]
            for i in range(inputs['ids'].size()[0]):
                tokens = inputs['ids'][i].cpu().numpy()
